...
{
	if(f>10.0 || f<0.01)
		pd_error(x, "lfoFreq value must be >= 0.01 Hz and <= 10.0 Hz.");
	else
		x->x_modSpeed = f;
}
...
static void delay_tilde_modDepth(t_delay_tilde *x, t_floatarg d)
{
	...
	if(d>100.0 || d<0.0)
		pd_error(x, "lfoDepth value must be >= 0.0%% and <= 100.0%%.");
	else
	{
...
static void delay_tilde_modShape(t_delay_tilde *x, t_floatarg s)
{
	...
	if(s>5.0 || s<0.0)
		pd_error(x, "lfoShape value must be either 0, 1, 2, 3, 4, or 5.");
	else
	{
		x->x_modShape = s/5.0;
...
{
	if(p>180.0 || p < -180.0)
		pd_error(x, "lfoPhase value must be >= -180 deg and <= 180 deg.");
	else
...
{
	if(f>1 || f<0)
		pd_error(x, "feedbackSwitch value must be 0 or 1.");
	else
...
{
	if(f>1 || f<0)
		pd_error(x, "freeze value must be 0 or 1.");
	else
	x->x_freeze = (int)f;
}
...
{
	if(h>2.0 || h<1.0)
		pd_error(x, "head value must be 1 or 2.");
	else
		x->x_twoHead = (int)h-1;
}
